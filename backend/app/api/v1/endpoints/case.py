...
async def create_case_message(
    ...
    # Optionally treat sender as seen implicitly by creating MP row (not required if unseen excludes own msgs)
    # We will skip creating MessagePerson here to avoid extra writes.
...
async def unseen_message_count(
    ...
    q = select(sa.func.count()).where(sa.and_(M.case_id == pk, M.written_by_id != pid, sa.not_(exists(subq))))
